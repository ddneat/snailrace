...
function FloorController(createCaption, floor_width, floor_height, finPosZ) {
	...
	function createTrackCaption() {
		...
		var startCaptionFront = new floor("", lineSize, linePivotCenter, 0.01, opacity, repeat = { x: lineSize.width / 5, y: lineSize.height / 5 });
		startCaptionFront.addPlaneToScene();
		// start: behind snail
		linePivotCenter = { x: lineSize.width / 2, y: lineSize.height / 2 + 1, z: 0 };
		var startCaptionBehind = new floor("", lineSize, linePivotCenter, 0.01, opacity, repeat = { x: lineSize.width / 5, y: lineSize.height / 5 });
		startCaptionBehind.addPlaneToScene();

		// finish	
		linePivotCenter = { x: lineSize.width / 2, y: finPosZ, z: 0 };
		var finishLine = new floor("", lineSize, linePivotCenter, 0.01, opacity, repeat = { x: lineSize.width / 5, y: lineSize.height / 5 });
		...
		for (var i = 0; i < trackAmount; i++) {
			createCaption(i + 1, fontheight, fontsize, position = { x: trackWidth * i + trackWidth / 2, y: 0, z: 1 }, rotation = { x: -Math.PI / 2, y: 0, z: 0 }, 16777215, 0.9, "trackCaption" + i, lambert = true, shadow = { receiveShadow: true, castShadow: false });
		}
	}
	...
	var soil = new floor("gras.jpg", soilSize, soilPivotCenter, -0.01, 1, repeat = { x: soilSize.width / 5, y: soilSize.height / 5 });
	...
	var track = new floor("floor_comic.jpg", trackSize, trackPivotCenter, 0, 1, repeat = { x: trackSize.width / 5, y: trackSize.height / 5 });
